...
from src.crypto.entity.config_entity import PredictionPipelineConfig
from src.crypto.entity.s3_estimator import SensorEstimator
    ...
    def predict(self,dataframe)->np.ndarray:
        try:
            ...
            if not is_model_present:
                raise cryptoException(f"Model not found at {self.prediction_pipeline_config.model_file_path} in bucket {self.prediction_pipeline_config.model_bucket_name}", sys)
...
if __name__ == "__main__":
    try:
        ...
        result = pipeline.initiate_prediction()
