...
import boto3
from src.crypto.configuration.aws_connection import S3Client
...
class SimpleStorageService:
    ...
    def read_object(object_name: object, decode: bool = True, make_readable: bool = False) -> Union[StringIO, str]:
        ...
        try:
            ...
            if object_name is None:
                raise cryptoException("Object is None. Cannot read from None object.", sys)
            ...
            if isinstance(object_name, list):
                ...
                if len(object_name) > 0:
                    ...
                    raise cryptoException("Empty list of objects provided", sys)

            # Check if object has get method
            if not hasattr(object_name, 'get'):
                raise cryptoException(f"Object of type {type(object_name)} does not have 'get' method", sys)

            # Get the object content
            response = object_name.get()
            if not response or 'Body' not in response:
                raise cryptoException("Invalid response from S3 object get() method", sys)
    ...
    def get_file_object( self, filename: str, bucket_name: str) -> Union[List[object], object]:
        ...
        try:
            ...
            if bucket is None:
                raise cryptoException(f"Bucket {bucket_name} not found", sys)
            ...
            if len(file_objects) == 0:
                ...
                raise cryptoException(f"No objects found with prefix {filename} in bucket {bucket_name}", sys)
